...
env.workspace = "//166.2.126.25/R4_VegMaps/R3_RiparianMapping/2018_forests/carson/deliverables/stage_5_table_cleanup/stage_5_table_cleanup.gdb"
fc = '//166.2.126.25/R4_VegMaps/R3_RiparianMapping/2018_forests/carson/deliverables/stage_5_table_cleanup/stage_5_table_cleanup.gdb/car_postreview_2018_10_17_newzs_classassign_dslv_dlsv_rename'
...
fieldsVH = ['height_mean', 'Size_Class', 'Data_Source', 'new_sc']
...
with arcpy.da.UpdateCursor(fc, fieldsVH) as cursor:
    ...
    for row in cursor:
        if row[0] <= .5 and row[2] == "Lidar2017":
            row[1] = "1) 0-.5 meters"
        elif row[0] <= 5 and row[0] > .5 and row[2] == "Lidar2017":
            row[1] = "2) .5-5 meters"
        elif row[0] <= 12 and row[0] > 5 and row[2] == "Lidar2017":
            row[1] = "3) 5-12 meters"
        elif row[0] > 12 and row[2] == "Lidar2017":
            row[1] = "4) 12+ meters"
